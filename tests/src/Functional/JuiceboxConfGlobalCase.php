...
class JuiceboxConfGlobalCase extends JuiceboxCaseTestBase {
  ...
  public function testGlobalConf() {
    ...
    $this->assertText('The Juicebox configuration options have been saved', 'Custom global options saved.');
  ...
  public function voidtestGlobalTrans() {
    ...
    $this->assertText('The strings have been saved', 'Languagelist translation saved.');
    // Now check the resulting XML again as an anon user.
    $this->drupalLogout();
    $this->drupalGet('juicebox/xml/field/node/' . $node->id() . '/' . $this->instFieldName . '/full');
  ...
  public function testGlobalMultisize() {
    ...
    $this->assertText('Your settings have been saved.', 'Gallery configuration changes saved.');
